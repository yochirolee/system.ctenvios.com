...
import { Cog6ToothIcon, UserIcon, UserPlusIcon } from "@heroicons/react/24/outline";

export default function CustomersActions() {
	return (
		<div className=" top-16 mx-2 text-right z-40">
			<Menu as="div" className="relative z-40 inline-block text-left">
				<div>
					<Menu.Button className="inline-flex w-full justify-center rounded-md bg-blue-400  px-4 py-2 text-sm font-medium text-white hover:bg-opacity-80 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75">
						<Cog6ToothIcon className="h-6 w-6 px-4 text-white" />
						<ChevronDownIcon
						...
						<div className="px-1 py-1 ">
							<Menu.Item>
								{({ active }) => (
									...
									>
										{active ? (
											<UserPlusIcon className="mr-2 h-5 w-5 text-blue-500" aria-hidden="true" />
										) : (
											<UserPlusIcon className="mr-2 h-5 w-5 text-" aria-hidden="true" />
										)}
										Nuevo Cliente
									</button>
								)}
